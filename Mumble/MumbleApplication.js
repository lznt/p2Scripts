...
/*var _connectionInfo =
{
    host      : "efe.mumble.com",    // Change to IP if you want to test remote Murmur servers.
    port      : 8560,          // Default port for Murmur, see murmur.ini for changing this.
    password  : "rautaarajalle",             // Default password for Murmur is empty, see murmur.ini for changing this.
    channel   : "EFE Server/LOBBY",             // Default Murmur server will have one channel called "Root". Empty channel name is depicted as "Root" when connecting via MumblePlugin.
...
var _connectionInfo =
{
    ...
    channel   : "mumble-serveur.com public #1/wow",             // Default Murmur server will have one channel called "Root". Empty channel name is depicted as "Root" when connecting via MumblePlugin.
    outputMuted : false,        // True means your voice is sent after connecting, false means your output is muted.
    intputMuted : false         // True means voice should be sent to us from other client after connecting, false means server wont send us the voice packets.
};
...
var clientWidgetVisible = false;

// when click the button "Mumble" to hind and display the window of client.
function StartMumble(){
	if (_mumbleWidget == null)
	{
		_mumbleWidget = ui.LoadFromFile("local://StartMumble.ui");
		_mumbleButton = findChild(_mumbleWidget, "MumbleBtn");
		_mumbleButton.clicked.connect(mumbleButtonClicked);
		
		
		//mumbleProxy = new UiProxyWidget(_mumbleWidget);
		
		//ui.AddProxyWidgetToScene(mumbleProxy);
		//mumbleProxy.visible = true;
		//mumbleProxy.windowFlags = 0;
    
    	//mumbleProxy.x = 15;             // they should be caculated late
    	//mumbleProxy.y = 150 ;

		_mumbleWidget.visible = true;
		_mumbleWidget.pos = new QPoint(15,150);
		_mumbleWidget.windowFlags = Qt.CustomizeWindowHint;
		//_mumbleWidget.windowFlags = setWindowFlags(Qt::CustomizeWindowHint);
		
		
	}
}

function mumbleButtonClicked()
{
	clientWidgetVisible = ! clientWidgetVisible;
	    _widget.visible = clientWidgetVisible;
}
